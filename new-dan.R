...
## dan's latest app.R that is converted to standalone
library(bioDist)
library(gplots)
library(gtools)
library(limma)
...
#  outputRESULT$datloaded <- reactive(is.numeric(nrow(dat)))
#  outputOptions(output, "datloaded", suspendWhenHidden = FALSE)
  
#  outputRESULT$table <- renderDataTable({
    sel <- inputCONFIG$sel
...
#      outputRESULT$ma.plot <- renderPlot(plot.null())
#      outputRESULT$heatmap <- renderPlot(plot.null())
      data.frame(NULL)})
#    outputRESULT$numsel <- reactive(length(c(ones, twos)))  
#    outputOptions(output, "numsel", suspendWhenHidden = FALSE)
    dat.sel <- dat[, c(ones, twos, "symbol")]
    if (inputCONFIG$summary == "AVE"){
    ...
    dat.sel$color <- "black"
    dat.sel$color[dat.sel$symbol == mousecase(inputCONFIG$gene1) | rownames(dat.sel) == mousecase(inputCONFIG$gene1)] <- 
      tolower(inputCONFIG$col1)
    dat.sel$color[dat.sel$symbol == mousecase(inputCONFIG$gene2) | rownames(dat.sel) == mousecase(inputCONFIG$gene2)] <- 
      tolower(inputCONFIG$col2)
    dat.sel$color[dat.sel$symbol == mousecase(inputCONFIG$gene3) | rownames(dat.sel) == mousecase(inputCONFIG$gene3)] <- 
      tolower(inputCONFIG$col3)
    dat.sel$color[dat.sel$symbol == mousecase(inputCONFIG$gene4) | rownames(dat.sel) == mousecase(inputCONFIG$gene4)] <- 
      tolower(inputCONFIG$col4)
    dat.sel$color[dat.sel$symbol == mousecase(inputCONFIG$gene5) | rownames(dat.sel) == mousecase(inputCONFIG$gene5)] <- 
      tolower(inputCONFIG$col5)
    dat.sel$color[dat.sel$symbol == mousecase(inputCONFIG$gene6) | rownames(dat.sel) == mousecase(inputCONFIG$gene6)] <- 
      tolower(inputCONFIG$col6)
...
#    outputRESULT$ma.plot <- renderPlot({
      ma.plot <- function(){
        par(mar = c(5, 5, 5, 8))
...
#    outputRESULT$heatmap <- renderPlot( {
      heatmap <- function(){
        ...
        if (inputCONFIG$heatscale %in% c("MC", "Z")) dat.heat <- sweep(dat.heat, 1, rowMeans(dat.heat))
        if (inputCONFIG$heatscale == "Z") dat.heat <- sweep(dat.heat, 1, apply(dat.heat, 1, sd), "/")
...
### XXX
        ...
        heatmap.22(t(dat.heat),
          ...
          distfun.col = function(...) {
            if (inputCONFIG$distfun == "E") return(dist(...))
            if (inputCONFIG$distfun == "AC") return(cor.dist(..., abs = T))
            if (inputCONFIG$distfun == "C") return(cor.dist(..., abs = F))},
          labCol = dat.top$symbol, colCol = dat.top$color)
      }
      heatmap()
#    })
      
#    outputRESULT$download.heatmap <- downloadHandler("Facebase_Microarray_Heatmap.png",
#      function(file) file.copy("heatmap.png", file))
      
    rownames(top) <- NULL
 #   outputRESULT$download.table <- downloadHandler("Facebase_Microarray_Table.csv",
 #     function(file) write.csv(as.data.frame(top), file, row.names = F))
    ...
    top <- formatStyle(datatable(top), "Color", target = "row", 
      color = styleEqual(
        c(0, inputCONFIG$col1, inputCONFIG$col2, inputCONFIG$col3, inputCONFIG$col4, inputCONFIG$col5),
        c("white", inputCONFIG$col1, inputCONFIG$col2, inputCONFIG$col3, inputCONFIG$col4, inputCONFIG$col5)))
#  })
    
#  outputRESULT$bullseyes <- renderPlot(
{
  bullseyes <- function(){
    genes <- c(inputCONFIG$gene1, inputCONFIG$gene2, inputCONFIG$gene3, inputCONFIG$gene4, inputCONFIG$gene5, inputCONFIG$gene6)
    good.genes <- which(genes %in% dat$symbol | genes %in% dat$probeset)
    genes <- genes[good.genes]
    if (length(genes) == 0) return(plot.null)
    colors <- c(inputCONFIG$col1, inputCONFIG$col2, inputCONFIG$col3, inputCONFIG$col4, inputCONFIG$col5, inputCONFIG$col6)
    ...
    exprs <- data.frame(
      ...
      stringsAsFactors = F)
    if (inputCONFIG$summary %in% c("AVE")){
      exprs.full <- exprs
    ...
    if (inputCONFIG$basecol == "mean of ages")
      exprs[, -(1:2)] <- sweep(exprs[, -(1:2)], 1, rowMeans(exprs[, -(1:2)]))
    if (inputCONFIG$basecol == "E10.5") exprs[, -(1:2)] <-
...
#  plotPNG(bullseyes, "bullseyes.png", width = 2000, height = 1000, res = 300)
 ...
 height = function(){
        ...
        good.genes <- which(genes %in% dat$symbol | genes %in% dat$probeset)
        genes <- genes[good.genes]
        if (inputCONFIG$summary == "Z") num.probesets <- length(dat$probeset[dat$symbol %in% genes]) else
...
#  outputRESULT$download.bullseyes <- downloadHandler("Facebase_Microarray_Bullseyes.png", 
